import { CreateTableCommand, DynamoDBClient, TagResourceCommand } from "@aws-sdk/client-dynamodb";
import { loadGlobalConfig, saveGlobalConfig } from "./config.js";

export async function createNewAccount(input: {name: string, region: string}){
    ...
    globalConfig.accounts.push({ name, defaultRegion: region, tableName: `${name}-spin-account` });
    await saveGlobalConfig(globalConfig);

    return {name, defaultRegion: region, tableName: `${name}-spin-account`};
}